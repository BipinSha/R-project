...
#	endDate - the end date of transcation, the records that happed after the end date will be removed. 
# It works with the start #date to set a time scope
...
setwd("C:\\Users\\bipin\\OneDrive\\Documents\\R-project\\RFM Analysis")
dir()


# read CDNOW_SAMPLE.txt
#df <- read.table(file.choose(),header=F)
df <- read.csv("CDNOW.csv")
head(df)
names(df)
...
### The Customer LTV (during the period of 2 months) is 63.21893
